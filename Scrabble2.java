...
class Word_generator {
	...
	TreeMap<Integer, String> permutate(String word, List<String> charList){
		 
		 charList.add(word);
		 
		 Set<String> possible_permutations = generate_permutations(charList);
		 int size = possible_permutations.size();
		 Scrabble obj = new Scrabble();
		 TreeMap<Integer, String> tm = new TreeMap<Integer, String>();
				 
		 for( String newWord : possible_permutations ){
			 if(obj.is_valid(newWord)) {
				 
				 String anagrams = obj.get_anagrams(newWord);
				 String[] arrayOfAnagrams = anagrams.split(" ");
				 String newAnagrams = "";
				 for( String validAnagram : arrayOfAnagrams){
					 if(validAnagram.contains(word)){
						 newAnagrams = newAnagrams + validAnagram + " ";
					 }
				 }
				 
				 Score_sort ss = new Score_sort("");
				 
				 String[] newArrayOfAnagrams = newAnagrams.split(" ");
				 for( String finalAnagrams : newArrayOfAnagrams){
					 int score = ss.calculate_score(finalAnagrams);
					 if(tm.containsKey(score)) {
						 tm.put(score,finalAnagrams);
					 }
					 else {
						 tm.put(score, tm.get(score) +" " + finalAnagrams);
					 }
				}
